...
const PropertyDetails = () => {
  ...
  useEffect(() => {
    ...
    axios.get(`http://localhost:8000/propertydetail/${id}/`)
      .then((res) => {
        ...
        if (prop.co_living) {
          axios.get(`http://localhost:8000/occupancy/?property=${prop.id}&is_active=true`)
            .then(occupancyRes => {
  ...
  return (
    <div className="container my-5">
      {/* ... (keep all existing header and navigation code unchanged) ... */}
      ...
      <div className="d-flex flex-column flex-md-row border p-4 rounded shadow-lg property-detail-container">
        ...
        <div className="col-md-6 ps-md-4 pt-3 pt-md-0">
          ...
          {/* ... (keep all existing property features section) ... */}
        <div className="d-flex flex-wrap gap-3 align-items-center my-3">
          ...
          {/* Add Tenants Living Section */}
          {property.co_living && currentOccupants.length > 0 && (
            <div className="mt-3">
              <h5>Tenants Living Here:</h5>
              <div className="d-flex flex-wrap gap-2">
                {currentOccupants.map(tenant => (
                  ...
                  >
                    <img 
                      src={tenant.user_image ? `http://localhost:8000${tenant.user_image}` : '/default-avatar.png'} 
                      alt={tenant.name}
          ...
          {/* ... (keep all existing property details sections) ... */}
          {/* Property Detail Section */}
                <div className="d-flex flex-column flex-md-row border p-4 rounded shadow-lg property-detail-container">
                  <div className="col-md-6">
                    <img
          ...
          <Modal show={showTenantModal} onHide={() => setShowTenantModal(false)} centered>
            ...
            <Modal.Body>
              {selectedTenant ? (
                <div className="text-center">
                  <img
                    src={selectedTenant.user_image ? `http://localhost:8000${selectedTenant.user_image}` : '/default-avatar.png'}
            ...
            <Modal.Footer>
              <Button variant="secondary" onClick={() => setShowTenantModal(false)}>Close</Button>
            </Modal.Footer>
          </Modal>
          ...
          {/* ... (keep all existing owner modal and booking sections) ... */}
        </div>
      </div>

      {/* ... (keep all existing related properties section) ... */}
